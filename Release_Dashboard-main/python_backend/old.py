    ...
    def __call__(self, func):
        ...
        def wrapper(cluster_name, env, timestamp):
            ...
            if current_time > (cache_timestamp + CACHE_DURATIONS[env]):
                self.cache[env].clear()  # Clear expired cache
                self.last_access_time[env] = current_time  # Reset timestamp
        ...
        for ns in namespaces.items:
            ...
            for deployment in deployments.items:
                deployment_info = {
                    ...
                    "time": current_time,
                    "cache_created_at": datetime.fromtimestamp(cache_timestamp).strftime("%I:%M %p"),
                    "cache_expires_at": datetime.fromtimestamp(cache_timestamp + cache_duration).strftime("%I:%M %p")
                }
                cluster_info.append(deployment_info)
        
        return {"status": "success", "data": cluster_info, "time": current_time}
